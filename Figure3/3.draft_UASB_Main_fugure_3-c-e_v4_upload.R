...
data.plot <- read.csv("D:/R_analysis/UASB_Mother/mother_draft_v2/xyplot/Figure3_data.csv", header=T, sep=",")
...
p_SS = 
  ggplot(data.plot,aes(x=day)) + 
  geom_line(aes(y=Reactor_SS),color="black",size=1.2) +
  xlab("Day")+
  ylab("Biomass (mg/L)") +     
  #geom_vline(xintercept=35,color="gray45",size=1,linetype = "dashed")+ 
  #geom_vline(xintercept=72,color="gray45",size=1,linetype = "dashed")+ 
  theme(panel.grid = element_blank(), 
        ...
        legend.key = element_rect(fill = 'transparent'))+
  scale_x_continuous(limits = c(0, 110),breaks=seq(0,110,20)) + # x從0到110 間隔10
  scale_y_continuous(limits = c(0, 12000),breaks=seq(0,12000,3000)) + # x從0到110 間隔10
  theme(axis.text.y = element_text(colour = "black", size = 16), 
        axis.text.x = element_text(colour = "black", size = 16),
        #axis.text.x = element_blank(),#hide label of x axis
        legend.text = element_text(size = 12, colour ="black"), 
...
setwd(save.wd)
ggsave("Figure3c-e_v4.png", width = 6.5, height = 10.1)
